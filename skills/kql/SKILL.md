...
Use the `read_skill_file` tool with `skill: "kql"` and the file path shown below.  
Example: `read_skill_file(skill="kql", path="references/operators.md")`

## Workflow Routing

Read the matching workflow file before responding — it contains step-by-step instructions.
...
The `samples/` directory contains production-ready `.kql` files organized by service. Use `read_skill_file` to read a relevant sample before writing a similar query — they demonstrate the expected format and conventions.
